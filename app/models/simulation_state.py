...
class SimulationState(BaseModel):
    ...
    pack_years: float = Field(default=0.0, ge=0, le=150, description="Paquetes-año acumulados")
    dieta: Literal["saludable", "normal", "mala"] = Field(default="normal")
    
    # Estado del Tumor (valores de la simulación matemática)
    volumen_tumor_sensible: float = Field(..., ge=0, description="Volumen células sensibles (cm³)")
    volumen_tumor_resistente: float = Field(default=0.0, ge=0, description="Volumen células resistentes (cm³)")
    
    # Tratamiento Actual
    tratamiento_activo: Literal["ninguno", "quimio", "radio", "inmuno"] = Field(default="ninguno")
    dias_tratamiento: int = Field(default=0, ge=0, description="Días acumulados de tratamiento")
    
    # Contexto de Modo
    modo: Literal["libre", "biblioteca"] = Field(default="libre")
    caso_id: str | None = Field(default=None, description="ID del caso si modo=biblioteca")
...
class TeacherResponse(BaseModel):
    ...
    fuentes: list[str] = Field(default_factory=list, description="Referencias médicas (NCCN, estudios)")
    advertencia: str | None = Field(default=None, description="Disclaimer educativo si aplica")
    
    # Metadata (opcional, para debugging)
    retrieved_chunks: int = Field(default=0, description="Número de chunks RAG recuperados")
    llm_model: str = Field(default="mock", description="Modelo LLM usado", alias="model_used")
